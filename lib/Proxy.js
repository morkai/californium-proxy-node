...
Proxy.prototype.request = function(message)
{
  ...
  var packet = new Buffer(4 + requestFrame.length);

  packet.writeUInt16BE(2 + requestFrame.length, 0);
  packet.writeUInt16BE(requestId, 2);
  requestFrame.copy(packet, 4);
...
Proxy.prototype.handleReaderData = function()
{
  ...
  if (this.currentFrameLength === 0 && this.reader.length >= 2)
  {
    this.currentFrameLength = this.reader.shiftUInt16();
  }
