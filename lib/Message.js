...
/**
 * Sets the specified option an array of options with the same option number.
 *
...
Message.prototype.getUriQuery = function()
{
  ...
  return uriQueryOptions.map(encodeURIComponent).join('&');
};
...
Message.prototype.setUriQuery = function(uriQuery)
{
  ...
  else
  {
    ...
    {
      this.addOption(Option.fromValue(optionRegistry.uriQuery, uriQuery[key]));
    }
  }
